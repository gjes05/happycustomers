...
REGION_PROFILES = [
    {
        ...
        "type": "anomaly_good", # A special type
        "network": {"latency_range": (20.0, 80.0), "loss_range": (0.0, 0.5), "spike_chance": 0.01}, # Good network
        "prompt_bias": "The sentiment should be surprisingly neutral or negative, despite good signal. Topics are about billing issues, poor customer service, or confusing promotions. Users are frustrated with the company, not the signal."
    },
    {
        "name": "New York (Good)",
        "type": "good",
...
def main():
    ...
    # --- Config ---
    TICK_INTERVAL_SECONDS = 5 # Time between data generation ticks
    ...
    model = genai.GenerativeModel('gemini-2.5-flash-lite') # Using Flash for speed
    
    print(f"--- 🚀 T-Mobile Real-Time Simulator START 🚀 ---")
    print(f"Generating new data every {TICK_INTERVAL_SECONDS} seconds...")
    ...
    try:
        while True:
            ...
            for region in REGION_PROFILES:
                ...
                if tick_count % 3 == 0:
                    print(f"Generating Tweet for {region['name']}...")
                    tweet = generate_tweet(model, region)
                    if tweet:
                        all_events.append(tweet)
                
                if tick_count % 10 == 0:
                    print(f"Generating Support Log for {region['name']}...")
